...
export function ExpenseAutoEntryConfirmation({ draftId }: { draftId: string }) {
  ...
  const reviewSource = useMemo(
    () => draft ? persistedAutoEntryOriginalToSource(draft.autoEntry.original) : null,
    [draft],
  );
  ...
  if (!draft || !form || !reviewSource) return <main className="p-4 md:p-8"><Card><CardContent className="text-destructive">{error ?? "申請内容を読み込めませんでした。"}</CardContent></Card></main>;
