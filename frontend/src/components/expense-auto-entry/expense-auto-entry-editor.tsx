...
export function ExpenseAutoEntryEditor({
  ...
  onEvidenceFieldFocus?: (path: string | null) => void;
  onAddItem: () => void;
  ...
  return <div className="space-y-6">
    ...
    <section className="space-y-3"><h2 className="font-semibold">請求書・注文書の読み取り値</h2>{documentFields.map((documentField) => {
      ...
      return <div className="rounded-md border p-3" key={documentField.path}><label className="grid gap-1 text-sm">{documentField.label}<Input maxLength={documentField.path === "document.issuerTaxRegistrationNumber" ? 100 : 500} min={documentField.type === "number" ? 1 : undefined} onBlur={() => onEvidenceFieldFocus?.(null)} onChange={(event) => onDocumentChange(documentField.path === "document.issuerName" ? { issuerName: event.target.value } : documentField.path === "document.issuerTaxRegistrationNumber" ? { issuerTaxRegistrationNumber: event.target.value } : { invoiceTotalAmount: event.target.value === "" ? null : Number(event.target.value) }, documentField.path)} onFocus={() => onEvidenceFieldFocus?.(documentField.path)} step={documentField.type === "number" ? 1 : undefined} type={documentField.type} value={documentField.value} /></label><FieldMetadata field={resolved.field} resolution={resolved.resolution} />{isReviewConfirmation ? <label className="mt-2 flex items-center gap-2 text-sm"><Checkbox checked={confirmedPaths.has(documentField.path)} onCheckedChange={(checked) => onConfirmationChange(documentField.path, checked === true)} />原本を確認しました</label> : null}</div>;
    })}</section>
    <section className="space-y-4"><h2 className="font-semibold">経費申請の入力</h2><div className="grid gap-3 md:grid-cols-2"><label className="grid gap-1 text-sm">経費区分<select className="h-8 rounded-md border bg-background px-2 text-sm" onChange={(event) => onApplicationChange({ category: event.target.value as ExpenseCategory })} value={form.application.category}>{expenseCategories.map((category) => <option key={category} value={category}>{categoryLabels[category]}</option>)}</select></label><label className="grid gap-1 text-sm"><span>利用日 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(event) => onExpenseDateChange(event.target.value)} required type="date" value={form.application.expenseDate} /></label><label className="grid gap-1 text-sm md:col-span-2"><span>件名 <span aria-hidden="true" className="text-destructive">*</span></span><Input maxLength={200} onChange={(event) => onApplicationChange({ title: event.target.value })} required value={form.application.title} /></label><label className="grid gap-1 text-sm md:col-span-2"><span>利用目的 <span aria-hidden="true" className="text-destructive">*</span></span><textarea className="min-h-20 rounded-md border bg-background p-2 text-sm" onChange={(event) => onApplicationChange({ purpose: event.target.value })} required value={form.application.purpose} /></label><label className="grid gap-1 text-sm md:col-span-2">備考<textarea className="min-h-16 rounded-md border bg-background p-2 text-sm" onChange={(event) => onApplicationChange({ remarks: event.target.value })} value={form.application.remarks} /></label></div></section>
    <section className="space-y-3"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">経費明細</h2><Button onClick={onAddItem} size="sm" type="button" variant="outline"><Plus data-icon="inline-start" />明細追加</Button></div>{form.application.items.map((item, index) => {
      const paths = getAutoEntryLineItemFieldPaths(item.sourceLineItemIndex);
      const description = paths ? resolvedByPath.get(paths.descriptionPath) : undefined;
      const amount = paths ? resolvedByPath.get(paths.amountPath) : undefined;
      return <div className="space-y-3 rounded-md border p-3" key={`${item.sourceLineItemIndex ?? "manual"}-${index}`}><div className="grid gap-3 md:grid-cols-2"><label className="grid gap-1 text-sm"><span>内容 <span aria-hidden="true" className="text-destructive">*</span></span><Input maxLength={500} onBlur={() => onEvidenceFieldFocus?.(null)} onChange={(event) => onItemChange(index, { description: event.target.value })} onFocus={() => onEvidenceFieldFocus?.(description?.path ?? null)} required value={item.description} /></label><label className="grid gap-1 text-sm"><span>金額（円） <span aria-hidden="true" className="text-destructive">*</span></span><Input min={1} onBlur={() => onEvidenceFieldFocus?.(null)} onChange={(event) => onItemChange(index, { amount: Number(event.target.value) })} onFocus={() => onEvidenceFieldFocus?.(amount?.path ?? null)} required step={1} type="number" value={item.amount || ""} /></label></div>{description ? <><FieldMetadata field={description.field} resolution={description.resolution} />{description.field.status === "REVIEW" && description.resolution !== "EDITED" ? <label className="flex items-center gap-2 text-sm"><Checkbox checked={confirmedPaths.has(description.path)} onCheckedChange={(checked) => onConfirmationChange(description.path, checked === true)} />内容の原本を確認しました</label> : null}</> : null}{amount ? <><FieldMetadata field={amount.field} resolution={amount.resolution} />{amount.field.status === "REVIEW" && amount.resolution !== "EDITED" ? <label className="flex items-center gap-2 text-sm"><Checkbox checked={confirmedPaths.has(amount.path)} onCheckedChange={(checked) => onConfirmationChange(amount.path, checked === true)} />金額の原本を確認しました</label> : null}</> : null}{form.application.category === "MEAL" || form.application.category === "TRAINING" || form.application.category === "CERTIFICATION" ? <label className="grid gap-1 text-sm"><span>{form.application.category === "MEAL" ? "店舗名" : form.application.category === "TRAINING" ? "主催者" : "試験実施団体"} <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { merchantName: event.target.value })} required value={item.merchantName} /></label> : null}{form.application.category === "MEAL" ? <label className="grid gap-1 text-sm"><span>参加者 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { participants: event.target.value })} required value={item.participants} /></label> : null}{form.application.category === "TRANSPORTATION" ? <div className="grid gap-3 md:grid-cols-3"><label className="grid gap-1 text-sm"><span>交通手段 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { transportationType: event.target.value })} required value={item.transportationType} /></label><label className="grid gap-1 text-sm"><span>出発地 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { origin: event.target.value })} required value={item.origin} /></label><label className="grid gap-1 text-sm"><span>到着地 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { destination: event.target.value })} required value={item.destination} /></label></div> : null}<div className="flex justify-end"><Button aria-label={`明細${index + 1}を削除`} onClick={() => onDeleteItem(index)} size="sm" type="button" variant="ghost"><Trash2 data-icon="inline-start" />削除</Button></div></div>;
