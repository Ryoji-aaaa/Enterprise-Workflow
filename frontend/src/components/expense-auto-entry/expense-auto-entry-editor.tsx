...
export function ExpenseAutoEntryEditor({
  ...
  return <div className="space-y-6">
    ...
    <section className="space-y-4"><h2 className="font-semibold">経費申請の入力</h2><div className="grid gap-3 md:grid-cols-2"><label className="grid gap-1 text-sm">経費区分<select className="h-8 rounded-md border bg-background px-2 text-sm" onChange={(event) => onApplicationChange({ category: event.target.value as ExpenseCategory })} value={form.application.category}>{expenseCategories.map((category) => <option key={category} value={category}>{categoryLabels[category]}</option>)}</select></label><label className="grid gap-1 text-sm"><span>利用日 <span className="text-destructive">*</span></span><Input onChange={(event) => onExpenseDateChange(event.target.value)} required type="date" value={form.application.expenseDate} /></label><label className="grid gap-1 text-sm md:col-span-2"><span>件名 <span className="text-destructive">*</span></span><Input maxLength={200} onChange={(event) => onApplicationChange({ title: event.target.value })} required value={form.application.title} /></label><label className="grid gap-1 text-sm md:col-span-2"><span>利用目的 <span className="text-destructive">*</span></span><textarea className="min-h-20 rounded-md border bg-background p-2 text-sm" onChange={(event) => onApplicationChange({ purpose: event.target.value })} required value={form.application.purpose} /></label><label className="grid gap-1 text-sm md:col-span-2">備考<textarea className="min-h-16 rounded-md border bg-background p-2 text-sm" onChange={(event) => onApplicationChange({ remarks: event.target.value })} value={form.application.remarks} /></label></div></section>
    <section className="space-y-3"><div className="flex items-center justify-between gap-3"><h2 className="font-semibold">経費明細</h2><Button onClick={onAddItem} size="sm" type="button" variant="outline"><Plus data-icon="inline-start" />明細追加</Button></div>{form.application.items.map((item, index) => {
      ...
      return <div className="space-y-3 rounded-md border p-3" key={`${item.sourceLineItemIndex ?? "manual"}-${index}`}><div className="grid gap-3 md:grid-cols-2"><label className="grid gap-1 text-sm"><span>内容 <span className="text-destructive">*</span></span><Input maxLength={500} onBlur={() => onEvidenceFieldFocus?.(null)} onChange={(event) => onItemChange(index, { description: event.target.value })} onFocus={() => onEvidenceFieldFocus?.(description?.path ?? null)} required value={item.description} /></label><label className="grid gap-1 text-sm"><span>金額（円） <span className="text-destructive">*</span></span><Input min={1} onBlur={() => onEvidenceFieldFocus?.(null)} onChange={(event) => onItemChange(index, { amount: Number(event.target.value) })} onFocus={() => onEvidenceFieldFocus?.(amount?.path ?? null)} required step={1} type="number" value={item.amount || ""} /></label></div>{description ? <><FieldMetadata field={description.field} resolution={description.resolution} />{description.field.status === "REVIEW" && description.resolution !== "EDITED" ? <label className="flex items-center gap-2 text-sm"><Checkbox checked={confirmedPaths.has(description.path)} onCheckedChange={(checked) => onConfirmationChange(description.path, checked === true)} />内容の原本を確認しました</label> : null}</> : null}{amount ? <><FieldMetadata field={amount.field} resolution={amount.resolution} />{amount.field.status === "REVIEW" && amount.resolution !== "EDITED" ? <label className="flex items-center gap-2 text-sm"><Checkbox checked={confirmedPaths.has(amount.path)} onCheckedChange={(checked) => onConfirmationChange(amount.path, checked === true)} />金額の原本を確認しました</label> : null}</> : null}{form.application.category === "MEAL" || form.application.category === "TRAINING" || form.application.category === "CERTIFICATION" ? <label className="grid gap-1 text-sm"><span>{form.application.category === "MEAL" ? "店舗名" : form.application.category === "TRAINING" ? "主催者" : "試験実施団体"} <span className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { merchantName: event.target.value })} required value={item.merchantName} /></label> : null}{form.application.category === "MEAL" ? <label className="grid gap-1 text-sm"><span>参加者 <span className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { participants: event.target.value })} required value={item.participants} /></label> : null}{form.application.category === "TRANSPORTATION" ? <div className="grid gap-3 md:grid-cols-3"><label className="grid gap-1 text-sm"><span>交通手段 <span className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { transportationType: event.target.value })} required value={item.transportationType} /></label><label className="grid gap-1 text-sm"><span>出発地 <span className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { origin: event.target.value })} required value={item.origin} /></label><label className="grid gap-1 text-sm"><span>到着地 <span className="text-destructive">*</span></span><Input onChange={(event) => onItemChange(index, { destination: event.target.value })} required value={item.destination} /></label></div> : null}<div className="flex justify-end"><Button aria-label={`明細${index + 1}を削除`} onClick={() => onDeleteItem(index)} size="sm" type="button" variant="ghost"><Trash2 data-icon="inline-start" />削除</Button></div></div>;
