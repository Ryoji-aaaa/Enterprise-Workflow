...
export function ExpenseApplicationDetail({
  ...
  const [application, setApplication] = useState<ExpenseApplication | null>(
    null,
  );
  ...
  async function action(path: string, body?: object) {
    ...
    try {
      ...
      if (!(cause instanceof AuthenticationRequiredError)) {
        setError(
          cause instanceof Error ? cause.message : "処理できませんでした。",
        );
      }
    } finally {
      setProcessing(false);
  ...
  if (!application && !error)
    return (
      <Card>
        <CardContent>申請を読み込んでいます…</CardContent>
      </Card>
    );
  if (!application)
    return (
    ...
    <div className="space-y-6">
      ...
      {application.returnReason && (
        <Card className="border-destructive/40">
          <CardHeader>
            <CardTitle className="text-destructive">差戻し理由</CardTitle>
          </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
          <div>
            <p className="text-muted-foreground">申請番号</p>
            <p>{application.applicationNumber}</p>
          </div>
          <div>
            <p className="text-muted-foreground">区分</p>
            <p>{categoryLabels[application.category]}</p>
          </div>
          <div>
            <p className="text-muted-foreground">利用日</p>
            <p>{application.expenseDate}</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-muted-foreground">申請時所属</p>
            <p>
              {application.divisionUnitName} /{" "}
              {application.organizationUnitName}
            </p>
          </div>
          <div>
            <p className="text-muted-foreground">合計</p>
            <p className="font-semibold">{yen(application.totalAmount)}</p>
          </div>
          <div className="sm:col-span-2 lg:col-span-3">
            <p className="text-muted-foreground">利用目的</p>
            <p className="whitespace-pre-wrap">{application.purpose}</p>
          </div>
          {application.remarks && (
            <div className="sm:col-span-2 lg:col-span-3">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-2xl text-left text-sm">
              <thead className="border-b text-muted-foreground">
                <tr>
                  <th className="p-2">利用日</th>
                  <th className="p-2">内容</th>
                  <th className="p-2">支払先・区間</th>
              ...
              <tbody className="divide-y">
                {application.items.map((item) => (
                  <tr key={item.id}>
                    <td className="p-2">{item.expenseDate}</td>
                    <td className="p-2">
                      {item.description}
                      {item.participants && (
                        <p className="text-xs text-muted-foreground">
                          参加者: {item.participants}
                        </p>
                      )}
                    </td>
                    <td className="p-2">
                    ...
                    <td className="p-2 text-right">{yen(item.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
