    ...
    <form className="space-y-6" onSubmit={onSubmit}>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <label className="grid gap-1 text-sm"><span>利用日 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(e) => setExpenseDate(e.target.value)} required type="date" value={expenseDate} /></label>
          <label className="grid gap-1 text-sm md:col-span-2"><span>件名 <span aria-hidden="true" className="text-destructive">*</span></span><Input maxLength={200} onChange={(e) => setTitle(e.target.value)} required value={title} /></label>
          <label className="grid gap-1 text-sm md:col-span-2"><span>利用目的 <span aria-hidden="true" className="text-destructive">*</span></span><textarea className="min-h-24 rounded-md border bg-background p-3" onChange={(e) => setPurpose(e.target.value)} required value={purpose} /></label>
        ...
        <CardContent className="space-y-4">
          {items.map((item, index) => (
            <div className="grid gap-3 rounded-lg border p-4 md:grid-cols-3" key={item.id ?? index}>
              <label className="grid gap-1 text-sm"><span>{itemDateLabel} <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(e) => updateItem(index, { expenseDate: e.target.value })} required type="date" value={item.expenseDate} /></label>
              <label className="grid gap-1 text-sm md:col-span-2"><span>{itemDescriptionLabel} <span aria-hidden="true" className="text-destructive">*</span></span><Input maxLength={500} onChange={(e) => updateItem(index, { description: e.target.value })} required value={item.description} /></label>
              <label className="grid gap-1 text-sm"><span>金額（円） <span aria-hidden="true" className="text-destructive">*</span></span><Input min={1} onChange={(e) => updateItem(index, { amount: Number(e.target.value) })} required step={1} type="number" value={item.amount || ""} /></label>
              <label className="grid gap-1 text-sm md:col-span-2"><span>{category === "MEAL" ? "店舗名" : category === "TRAINING" ? "主催者" : category === "CERTIFICATION" ? "試験実施団体" : "支払先"}{merchantNameRequired ? <> <span aria-hidden="true" className="text-destructive">*</span></> : null}</span><Input onChange={(e) => updateItem(index, { merchantName: e.target.value })} required={merchantNameRequired} value={item.merchantName ?? ""} /></label>
              {category === "MEAL" && <label className="grid gap-1 text-sm md:col-span-3"><span>参加者（社内／社外区分・人数を含む） <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(e) => updateItem(index, { participants: e.target.value })} required value={item.participants ?? ""} /></label>}
              {category === "TRANSPORTATION" && <>
                <label className="grid gap-1 text-sm"><span>交通手段 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(e) => updateItem(index, { transportationType: e.target.value })} required value={item.transportationType ?? ""} /></label>
                <label className="grid gap-1 text-sm"><span>出発地 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(e) => updateItem(index, { origin: e.target.value })} required value={item.origin ?? ""} /></label>
                <label className="grid gap-1 text-sm"><span>到着地 <span aria-hidden="true" className="text-destructive">*</span></span><Input onChange={(e) => updateItem(index, { destination: e.target.value })} required value={item.destination ?? ""} /></label>
              </>}
