...
export default function MailNotificationsPage() {
  ...
  useEffect(() => {
    ...
    fetchBackend(
      `/api/backend/admin/mail-notifications?${queryString(filters, page)}`,
      {
        cache: "no-store",
    ...
    <main className="p-4 md:p-8">
      <div className="mx-auto max-w-7xl">
            ...
            <form className="grid gap-3 md:grid-cols-3" onSubmit={search}>
                  ...
                  onChange={(event) =>
                    setDraft({
                      ...
                      notificationType: event.target
                        .value as Filters["notificationType"],
                    })
                  }
                >
                  <option value="">すべて</option>
              ...
              <label className="grid gap-1 text-sm">
                ...
                <Input
                  ...
                  onChange={(event) =>
                    setDraft({ ...draft, recipientEmail: event.target.value })
                  }
                />
              </label>
              <label className="grid gap-1 text-sm">
        ...
        {data && (
          <Card>
            <CardHeader>
              <CardTitle>通知履歴（{data.totalElements}件）</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full min-w-6xl text-left text-sm">
                  <thead className="border-b text-muted-foreground">
                    <tr>
                      <th className="p-3">送付日時</th>
                      <th className="p-3">状態</th>
                      <th className="p-3">種別</th>
                      <th className="p-3">宛先</th>
                      <th className="p-3">件名</th>
                      <th className="p-3">対象申請</th>
                      <th className="p-3">試行</th>
                  ...
                  <tbody className="divide-y">
                    {data.content.map((item) => (
                      <tr key={item.notificationId}>
                        ...
                        <td className="p-3">
                          <LinkButton
                            href={`/admin/mail-notifications/${item.notificationId}`}
                            variant="outline"
                          >
                            詳細
                          </LinkButton>
