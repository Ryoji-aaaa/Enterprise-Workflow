...
export default function MailNotificationDetailPage() {
  ...
  const [notification, setNotification] =
    useState<MailNotificationDetail | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const controller = new AbortController();
    fetchBackend(
      `/api/backend/admin/mail-notifications/${encodeURIComponent(notificationId)}`,
      {
        cache: "no-store",
      ...
      .then(async (response) => {
        if (response.status === 403)
          throw new Error("メール通知履歴を参照する権限がありません（403）。");
        if (response.status === 404)
          throw new Error("メール通知履歴が見つかりません（404）。");
        if (!response.ok)
          throw new Error("メール通知履歴を取得できませんでした。");
    ...
    <main className="p-4 md:p-8">
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex items-center justify-between gap-3">
          ...
          <LinkButton href="/admin/mail-notifications" variant="outline">
            一覧へ戻る
          </LinkButton>
        </div>
        {error && (
        ...
        {!error && !notification && (
          <Card>
            <CardContent>メール通知履歴を読み込んでいます…</CardContent>
          </Card>
        )}
        {notification && (
          <div className="grid gap-4">
            <Card>
              ...
              <CardContent>
                <dl className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <Entry label="宛先氏名">{notification.recipientName}</Entry>
                  <Entry label="宛先メールアドレス">
                  ...
                  <Entry label="試行回数">{notification.attemptCount}</Entry>
                  <Entry label="作成日時">
                  ...
                  <Entry label="次回試行日時">
                    {displayDate(notification.nextAttemptAt)}
                  </Entry>
                </dl>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <pre className="whitespace-pre-wrap break-words rounded-md bg-muted p-4 font-sans text-sm">
