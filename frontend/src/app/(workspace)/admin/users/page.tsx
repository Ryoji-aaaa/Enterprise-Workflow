...
export default function AdminUsersPage() {
  ...
  const canUpdate = currentUser.permissions.includes("USER_UPDATE");
  ...
  return (
    <main className="p-4 md:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        {!error && !users && (
          <Card>
            <CardContent>ユーザーを読み込んでいます…</CardContent>
          </Card>
        )}
        {users && (
          <Card>
            <CardHeader>
              <CardTitle>ユーザー一覧（{users.totalElements}件）</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <table className="w-full min-w-4xl text-left text-sm">
                  <thead className="border-b text-muted-foreground">
                    <tr>
                      ...
                      <th className="p-3">雇用区分</th>
                      <th className="p-3">状態</th>
                      <th className="p-3">主所属・役職</th>
                  ...
                  <tbody className="divide-y">
                    {users.content.map((user) => (
                      <tr key={user.id}>
                        ...
                        <td className="p-3">
                          {employmentLabels[user.employmentType]}
                        </td>
                        <td className="p-3">
              ...
              <div className="mt-4 flex items-center justify-end gap-3">
                ...
                <span className="text-sm">
                  {users.page + 1} / {Math.max(users.totalPages, 1)}
                </span>
                <Button
                  disabled={users.page + 1 >= users.totalPages}
                  onClick={() => setPage((value) => value + 1)}
                  variant="outline"
                >
                  次へ
