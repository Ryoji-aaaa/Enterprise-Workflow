...
async function api<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const error = new Error(body.message ?? "操作に失敗しました。") as Error & {
      status?: number;
    };
...
async function loadEditorData(userId: string) {
  const [user, assignments, roleAssignments, units, positions, roles, users] =
    await Promise.all([
      api<User>(`/api/backend/admin/users/${userId}`),
      api<Assignment[]>(
        `/api/backend/admin/users/${userId}/organization-assignments`,
      ),
...
export default function EditUserPage() {
  ...
  async function updateProfile(event: FormEvent<HTMLFormElement>) {
    ...
    const validUntil = form.get("validUntil")
      ? new Date(String(form.get("validUntil")))
      : null;
  ...
  async function changeStatus(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const updated = await api<User>(
        `/api/backend/admin/users/${userId}/status`,
        {
          method: "PATCH",
          body: JSON.stringify({
            status: form.get("status"),
            reasonCode: "ADMIN_UI",
            reasonText: form.get("reason"),
            effectiveAt: new Date().toISOString(),
          }),
        },
      );
      setUser(updated);
      setMessage("アカウント状態を更新しました。");
      setError(null);
    } catch (reason) {
      showFailure(reason);
    }
  ...
  async function endAssignment(assignment: Assignment) {
    try {
      await api(
        `/api/backend/admin/users/${userId}/organization-assignments/${assignment.id}?version=${assignment.version}&reason=${encodeURIComponent("管理画面から所属終了")}`,
        { method: "DELETE" },
      );
      setError(null);
      await reload();
      setMessage("所属を終了しました。");
    } catch (reason) {
      showFailure(reason);
    }
  }
  ...
  async function updateAssignment(
    ...
    try {
      await api(
        `/api/backend/admin/users/${userId}/organization-assignments/${assignment.id}`,
        {
          method: "PATCH",
          body: JSON.stringify({
            organizationUnitId: form.get("organizationUnitId"),
  ...
  async function revokeRole(assignmentId: string) {
    try {
      await api(
        `/api/backend/admin/users/${userId}/roles/${assignmentId}?reason=${encodeURIComponent("管理画面から剥奪")}`,
        { method: "DELETE" },
      );
      setError(null);
      await reload();
      setMessage("ロールを剥奪しました。");
    } catch (reason) {
      showFailure(reason);
    }
  }
  ...
  const unitName = (id: string) =>
    units.find((unit) => unit.id === id)?.unitName ?? id;
  const positionName = (id: string | null) =>
    positions.find((position) => position.id === id)?.name ?? "役職なし";
    ...
    <main className="p-4 md:p-8">
      <div className="mx-auto max-w-5xl space-y-5">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold">ユーザー情報編集</h1>
        ...
        {!user && !error && (
          <Card>
            <CardContent>ユーザー情報を読み込んでいます…</CardContent>
          </Card>
        )}
        {user && (
          <>
            <Card>
              <CardHeader>
                <CardTitle>基本情報・有効期間・雇用区分</CardTitle>
              </CardHeader>
              <CardContent>
                <form
                ...
                >
                  <label className="grid gap-1 text-sm">
                    email（変更不可）
                    <Input name="email" readOnly value={user.email} />
                  </label>
                  <label className="grid gap-1 text-sm">
                    社員番号
                    <Input
                      defaultValue={user.employeeCode ?? ""}
                      disabled={!permissions.includes("USER_UPDATE")}
                      maxLength={50}
                      name="employeeCode"
                    />
                  </label>
                  ...
                  <label className="grid gap-1 text-sm">
                    利用開始
                    <Input
                      defaultValue={user.validFrom.slice(0, 16)}
                      disabled={!permissions.includes("USER_UPDATE")}
                      name="validFrom"
                      type="datetime-local"
                    />
                  ...
                  <label className="grid gap-1 text-sm">
                    利用終了
                    <Input
                      defaultValue={user.validUntil?.slice(0, 16) ?? ""}
                      disabled={!permissions.includes("USER_UPDATE")}
                      name="validUntil"
                      type="datetime-local"
                    />
                ...
                {permissions.includes("USER_STATUS_CHANGE") ? (
                  ...
                  >
                    <select
                      ...
                      name="status"
                    >
                      {["ACTIVE", "SUSPENDED", "DISABLED", "RETIRED"].map(
                        (value) => (
                    ...
                    <Input
                      ...
                      required
                    />
                    <Button type="submit">状態を変更</Button>
                  </form>
                ) : (
            ...
            <Card>
              <CardHeader>
                <CardTitle>所属・役職・直属上司</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-3">
                  {assignments.map((assignment) =>
                    permissions.includes("ORGANIZATION_MANAGE") &&
                    !assignment.validUntil ? (
                      <form
                        className="grid gap-3 rounded-lg border p-3 md:grid-cols-2"
                        key={assignment.id}
                        onSubmit={(event) =>
                          void updateAssignment(event, assignment)
                        }
                      >
                        <select
                          className="h-9 rounded-md border bg-background px-3"
                          defaultValue={assignment.organizationUnitId}
                        ...
                        <select
                          className="h-9 rounded-md border bg-background px-3"
                          defaultValue={assignment.managerUserId ?? ""}
                          name="managerUserId"
                        >
                        ...
                        >
                          ...
                          <option value="TEMPORARY">一時所属</option>
                          <option value="ACTING">代行</option>
                        </select>
                        <Input
                        ...
                        />
                        <Input
                          defaultValue={assignment.validUntil ?? ""}
                          name="validUntil"
                          type="date"
                        />
                        <div className="flex gap-2 md:col-span-2">
                          <Button type="submit">所属情報を保存</Button>
                          <Button
                      ...
                      >
                        <p className="font-medium">
                          {unitName(assignment.organizationUnitId)} /{" "}
                          {positionName(assignment.positionId)}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {assignment.assignmentType}・上司:{" "}
                          {users.find(
                            (value) => value.id === assignment.managerUserId,
                          )?.displayName ?? "なし"}
                          ・{assignment.validFrom}〜
                          {assignment.validUntil ?? "継続中"}
                        </p>
                      </div>
                    ),
                  )}
            ...
            {permissions.includes("ROLE_READ") && (
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {roleAssignments.map((assignment) => (
                      ...
                      >
                        ...
                        {permissions.includes("ROLE_REVOKE") &&
                          !assignment.validUntil && (
                            <Button
                  ...
                  {permissions.includes("ROLE_ASSIGN") && (
                    <form className="flex gap-3" onSubmit={addRole}>
                      <select
                      ...
                      <Button type="submit">ロール付与</Button>
                    </form>
                  )}
                </CardContent>
              </Card>
