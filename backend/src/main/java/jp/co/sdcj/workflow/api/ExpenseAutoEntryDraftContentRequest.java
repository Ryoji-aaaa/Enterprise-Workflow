...
public record ExpenseAutoEntryDraftContentRequest(
    ...
    public ExpenseApplicationInput toInput() {
    ...
    public List<Integer> sourceLineItemIndexes() {
        return items.stream().map(Item::sourceLineItemIndex).toList();
    }

    public record Document(
            @Size(max = 500) String issuerName,
            @Size(max = 100) String issuerTaxRegistrationNumber,
            @Positive @Digits(integer = 12, fraction = 0) BigDecimal invoiceTotalAmount) {
    }
}
